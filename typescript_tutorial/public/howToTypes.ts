...
ninja = {
  ...
  belt: 'yellow',
  age: 40,
  // skills: [] // won't work because changing initial structure
}


/* EXPLICIT TYPES: declaring a type, but no value */
let u_name: string; // this will be a string, value set later
let u_age: number;
let u_isLoggedIn: boolean;
...
/* ARRAYS AGAIN */ 
let ninjas: string[]; // this will be a string array
// ninjas = [1,2,3] // won't work
ninjas = ['Anna', 'Berta', 'Carla'];
// initialize as empty array for the push method to work 
let people: string[] = []; // this will be a string array, which has no entries yet
people.push('Dora');
people.push('Ella');
...
/* UNION TYPES */
let misto: (string|number)[] = []; // this is an empty array, which can have values of different types (string, number)
misto.push(20);
misto.push('abc');
...
let uncertain_age: any = 25; // currently of value 25 (number type), but can still be any type
...
let person: { name: any, age: any, belt: any };
